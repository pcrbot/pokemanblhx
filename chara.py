...
class Roster:
    ...
    def update(self):
        importlib.reload(_blhx_data)
        self._roster.clear()
        for idx, names in _blhx_data.CHARA_NAME.items():
            for n in names:
...
class Chara:
    ...
    @property
    def name(self):
        return _blhx_data.CHARA_NAME[self.id][0] if self.id in _blhx_data.CHARA_NAME else _blhx_data.CHARA_NAME[UNKNOWN][0]
